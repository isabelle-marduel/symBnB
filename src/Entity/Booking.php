...
class Booking
{
     ...
     *                      message="La date d'arrivée doit être égale ou ultérieure à la date d'aujourd'hui")
     */
    private $startDate;
    ...
    public function prePersist() {
        ...
        if( empty($this->amount)) {
            // prix de l'annonce multiplié par le nombre de jours du séjour
            $this->amount = $this->ad->getPrice() * $this->getDuration();
        }
    }

    public function isBookableDates(){
        // 1) Il faut connaître les dates que l'on ne peut pas réserver
        $notAvailableDays = $this->ad->getNotAvailableDays();
        // 2) Il faut comparer les dates choisies avec les dates non réservables
